...
const store = {
	...
	load() {
		...
		if (serializedData) {
			...
			// Migration map for Chinese to English shell names
			const shellNameMigration = {
				'随机': 'Random',
				'裂纹': 'Crackle',
				'十字': 'Crossette',
				'菊花': 'Chrysanthemum',
			...
			// Migrate old Chinese shell name to English
			if (data.shell && shellNameMigration[data.shell]) {
				config.shell = shellNameMigration[data.shell];
			} else if (data.shell) {
...
function configDidUpdate() {
	...
	// Control WebGL fireworks based on config
	const webgl = window.webglFireworks || initWebGLFireworks();
	if (webgl) {
		if (config.webglFireworks) {
			webgl.start();
		} else {
			webgl.stop();
		}
...
const helpContent = {
	shellType: {
		...
		body: 'Set the firework launch type. Choose "Random" for a more exciting visual experience!'
	},
	shellSize: {
		header: 'Firework Size',
		body: 'Set the size of the fireworks. Firework sizes are modeled after real firework sizes. Larger fireworks have a larger range, more detail, and sometimes more complex effects. However, larger sizes also require more processing power and may cause lag.'
	},
	quality: {
		header: 'Quality',
		body: 'Set the overall graphics quality. If the animation is not running smoothly, try lowering the quality. High quality significantly increases the number of rendered sparks and may cause lag.'
	},
	skyLighting: {
		header: 'Sky Lighting',
		body: 'Set the background brightness when fireworks explode. If the background looks too bright on your screen, you can try setting it to "Dim" or "None".'
	},
	scaleFactor: {
		header: 'Scale',
		body: 'Set the size of all scalable fireworks, essentially moving you closer or farther away. For larger firework sizes, this can be convenient to reduce the scale, especially on phones or tablets.'
	},
	autoLaunch: {
		header: 'Auto Launch',
		body: 'Automatically launches firework sequences. You can sit back and enjoy the show, or disable it for full control.'
	},
	finaleMode: {
		header: 'Finale Mode',
		body: 'Launch intense bursts of fireworks intermittently. May cause lag and requires "Auto Launch" to be enabled.'
	},
	hideControls: {
		header: 'Hide Controls',
		body: 'Hide the translucent controls at the top of the screen. This is helpful for screenshots, recordings, or if you just want a more seamless experience. When hidden, you can still click the top right corner to reopen this menu.'
	},
	fullscreen: {
		header: 'Fullscreen',
	...
	longExposure: {
		...
		body: 'Experimental effect that preserves long streaks of light, similar to opening a camera shutter.'
	},
	webglFireworks: {
		header: 'WebGL Background',
		body: 'Enable a WebGL shader-based firework animation as a continuous background layer. Features heart-shaped particle effects with trails. Can be toggled on/off independently of the main fireworks.'
	}
};
...
const nodeKeyToHelpKey = {
	...
	webglFireworksLabel: 'webglFireworks'
};
...
const appNodes = {
	...
	webglFireworks: '.webgl-fireworks',
	webglFireworksLabel: '.webgl-fireworks-label',
...
function renderApp(state) {
	...
	appNodes.webglFireworks.checked = state.config.webglFireworks;
...
function getConfigFromDOM() {
	return {
		...
		webglFireworks: appNodes.webglFireworks.checked,
...
appNodes.webglFireworks.addEventListener('click', () => setTimeout(updateConfig, 0));
...
function randomShellName() {
	return Math.random() < 0.5 ? 'Chrysanthemum' : shellNames[(Math.random() * (shellNames.length - 1) + 1) | 0 ];
}
...
const shellTypes = {
	...
	'Chrysanthemum': crysanthemumShell,
...
// Usage: Add ?music=1 or ?music=2 to the URL
// Example: index.html?music=1 (Payphone from 00:00)
//          index.html?music=2 (HPNY from 00:44)
const backgroundMusicPlayer = {
	audio: null,
	musicConfig: {
		'1': { file: 'payphone.mp3', startTime: 0 },      // Start at 00:00
		'2': { file: 'hpny.mp3', startTime: 44 }          // Start at 00:44
	},
	
	init() {
		// Get music selection from query parameter (?music=1 or ?music=2)
		const urlParams = new URLSearchParams(window.location.search);
		const musicId = urlParams.get('music') || '1'; // Default to payphone
		const config = this.musicConfig[musicId];
		
		if (!config) {
			console.warn('Invalid music ID:', musicId);
			return;
		}
		
		// Create audio element
		this.audio = new Audio(`./music/${config.file}`);
		this.audio.loop = true;
		this.audio.volume = 0.5;
		this.audio.currentTime = config.startTime;
...
// Start experience from intro screen
function startExperience() {
	...
	// Start WebGL if enabled
	const webgl = window.webglFireworksInstance;
	if (webgl && store.state.config.webglFireworks) {
		webgl.start();
	}
}
...
function init() {
	...
	// Set up intro screen start button
	const startButton = document.getElementById('startButton');
	if (startButton) {
		startButton.addEventListener('click', startExperience);
	} else {
		// If no intro screen, start immediately (fallback)
		togglePause(false);
		const webgl = initWebGLFireworks();
		if (webgl && store.state.config.webglFireworks) {
			window.webglFireworksInstance = webgl;
			webgl.start();
		}
	}
	...
	// Initialize WebGL fireworks (but don't start yet - will start on button click)
	const webgl = initWebGLFireworks();
	if (webgl) {
		window.webglFireworksInstance = webgl;
	}
...
if (IS_HEADER) {
	...
	setLoadingStatus('❤---------------Preparing Fireworks---------------❤');
	setTimeout(() => {
